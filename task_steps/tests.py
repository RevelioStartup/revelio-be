...
class TaskStepDeletionTestCase(TestCase):
    def setUp(self):
        ...
        self.task_step1 = TaskStep.objects.create(name="Step 1", output="Output 1", description="Description 1", status="NOT_STARTED", step_order=1, task=self.task)
        self.task_step2 = TaskStep.objects.create(name="Step 2", output="Output 2", description="Description 2", status="ON_PROGRESS", step_order=2, task=self.task)
