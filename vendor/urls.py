...
urlpatterns = [
    ...
    path('event/<str:event_id>/', VendorEventListView.as_view(), name='vendor-event-list'),
    path('<int:pk>/status/', VendorStatusUpdateAPIView.as_view(), name='vendor-status-update'),
    path('photos/', PhotoCreateView.as_view(), name='photo-vendor-create'),
    path('photos/<int:pk>/', PhotoRetrieveUpdateDestroyView.as_view(), name='photo-vendor-retrieve-update-destroy'),
]
